...
in {
  ...
  home.file."${config.home.homeDirectory}/.config/ranger/rifle.conf".text = ''
    ...
    label editor, !mime ^text, !ext xml|json|csv|tex|py|pl|rb|rs|js|sh|php|dart  = ${editor} -- "$@"
    ...
    label trash = mkdir -p -- "${config.home.homeDirectory}/.local/share/ranger/trash"; mv -- "$@" "${config.home.homeDirectory}/.local/share/ranger/trash"
  '';
}
